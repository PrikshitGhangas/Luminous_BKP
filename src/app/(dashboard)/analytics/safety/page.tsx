  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#243356] pb-4">
        ...
        <div className="flex items-center gap-1.5 bg-[#0F1026] border border-[#243356] p-1 rounded-xl text-xs font-mono">
          {[
            { label: '7 Days', days: 7 },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="border-[#D4AF37]/50 bg-gradient-to-br from-[#131C38] via-[#0F1026] to-[#1C2541] text-[#F4F1DE] shadow-xl overflow-hidden relative">
          <div className="absolute top-0 right-0 h-16 w-16 bg-[#D4AF37]/10 rounded-bl-full pointer-events-none" />
          <CardContent className="p-5 space-y-2">
            <div className="flex items-center justify-between text-[11px] font-mono text-[#C5A059] uppercase font-bold">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-[#B8B5A3] font-mono">/ 100</span>
            ...
            <div className="h-2 w-full bg-[#243356] rounded-full overflow-hidden">
              <div
            ...
            <p className="text-[10px] text-[#B8B5A3] font-mono">
        ...
        <Card className="border-[#243356] bg-[#0F1026] text-[#F4F1DE] shadow-xl">
          <CardContent className="p-5 space-y-2">
            <div className="flex items-center justify-between text-[11px] font-mono text-[#B8B5A3] uppercase font-bold">
              <span>Incidents in Window</span>
              <Activity className="h-4 w-4 text-[#C5A059]" />
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-extrabold font-mono text-[#F4F1DE]">
                {report.totalIncidentsAnalyzed}
              </span>
              <span className="text-xs text-[#B8B5A3] font-mono">records</span>
            </div>
        ...
        <Card className="border-[#243356] bg-[#0F1026] text-[#F4F1DE] shadow-xl">
          <CardContent className="p-5 space-y-2">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-lg font-bold font-mono text-[#F4F1DE] truncate">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold font-mono text-[#F4F1DE] uppercase tracking-wider flex items-center gap-2">
            <Layers className="h-4 w-4 text-[#FFD700]" />
            <span>6 Risk Categories (AI-Generated Risk Indicators)</span>
          </h3>
          <Button asChild size="sm" variant="outline" className="h-7 text-xs font-mono border-[#243356] hover:border-[#D4AF37] text-[#FFD700] gap-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {report.categoryList.map((cat) => (
            <Card
              key={cat.category}
              className={cn(
                'border bg-[#0F1026] text-[#F4F1DE] shadow-md transition-all hover:border-[#D4AF37]',
            ...
            >
              <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between border-b border-[#243356]">
              ...
              <CardContent className="p-4 space-y-2.5 text-xs">
                <div className="flex items-center justify-between font-mono">
                  <span className="text-[#B8B5A3] text-[11px]">Risk Indicator Score:</span>
                  <span className="font-bold text-sm text-[#FFD700]">{cat.riskScore} / 100</span>
                </div>

                <div className="h-1.5 w-full bg-[#243356] rounded-full overflow-hidden">
                  <div
                    className={cn(
                      'h-full rounded-full',
                      cat.riskLevel === 'CRITICAL'
                ...
                <p className="text-[11px] text-[#B8B5A3] leading-relaxed">
                  {cat.description}
                </p>

                <div className="flex items-center justify-between pt-1 border-t border-[#243356] text-[10px] font-mono text-[#7A786B]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="border-[#243356] bg-[#0F1026] text-[#F4F1DE] shadow-xl">
          <CardHeader className="p-4 pb-3 border-b border-[#243356] bg-[#131C38] flex flex-row items-center justify-between">
            <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono text-[#B8B5A3]">Weekly Aggregation</span>
          </CardHeader>
                ...
                return (
                  <div key={wk.weekLabel} className="space-y-1 text-xs font-mono">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-[#F4F1DE] font-semibold">{wk.weekLabel}</span>
                      <span className="text-[#FFD700] font-bold">{total} incidents</span>
                    </div>

                    {/* Multi-segment stacked bar */}
                    <div className="h-3 w-full bg-[#1C2541] rounded-full overflow-hidden flex">
                      <div
                    ...
                    <div className="flex items-center justify-between text-[9px] text-[#7A786B]">
          ...
          <CardContent className="p-4 space-y-3.5">
            {report.responseTimeTrends.map((rt) => (
              <div key={rt.department} className="space-y-1 text-xs font-mono">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-[#F4F1DE] font-semibold">{rt.department}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-emerald-400 font-bold">{rt.avgResponseMinutes} min</span>
                    <span className="text-[10px] text-[#7A786B]">(Target: &lt; {rt.targetMinutes}m)</span>
                  </div>
                </div>

                <div className="h-2 w-full bg-[#1C2541] rounded-full overflow-hidden">
                  <div
                    className={cn(
                      'h-full rounded-full',
                ...
                <div className="flex items-center justify-between text-[9px] text-[#7A786B]">
                  ...
                  <span className="text-[#C5A059]">{rt.totalHandled} incidents dispatched</span>
                </div>
              </div>
            ))}
          </CardContent>
      ...
      <Card className="border-[#243356] bg-[#0F1026] text-[#F4F1DE] shadow-xl">
        <CardHeader className="p-4 pb-3 border-b border-[#243356] bg-[#131C38] flex flex-row items-center justify-between">
          ...
          <span className="text-[10px] font-mono text-[#B8B5A3]">24-Hour Campus Telemetry</span>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {report.timePatterns.map((tp) => (
              <div
                key={tp.hourWindow}
                className={cn(
                  'rounded-xl border p-3.5 space-y-2 bg-[#0B132B]',
              ...
              >
                <div className="flex items-center justify-between font-mono text-[10px]">
                  <span className="font-bold text-[#FFD700]">{tp.hourWindow}</span>
                  <span
                    className={cn(
                ...
                <div className="text-xs font-mono font-bold text-[#F4F1DE] flex items-center gap-1.5">
                  <span>{tp.incidentCount} historical events</span>
                  <span className="text-[10px] text-[#C5A059]">({tp.primaryCategory})</span>
                </div>
                <p className="text-[11px] text-[#B8B5A3] leading-relaxed">
                  {tp.description}
                </p>
      ...
      <div className="rounded-2xl border border-[#D4AF37]/50 bg-gradient-to-r from-[#131C38] via-[#0F1026] to-[#1C2541] p-6 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="space-y-1 text-center sm:text-left">
          <h4 className="text-sm font-bold font-mono text-[#F4F1DE] flex items-center gap-2 justify-center sm:justify-start">
        ...
        <Button asChild className="bg-[#D4AF37] hover:bg-[#C5A059] text-[#0B132B] font-bold font-mono text-xs px-5 h-10 gap-2 shrink-0">
          <Link href="/safety/risk-intelligence">
